...
import { TrendingDown, TrendingUp, Minus, BarChart3, Info } from 'lucide-react'
import { calcularTransicao, calcularAtual, REDUCAO, NOVOS } from '../data/aliquotas'
...
const MAX_BAR = 0.30 // 30% = 100% da barra
...
export default function Simulador() {
  ...
  const [visao, setVisao]               = useState('tabela') // 'tabela' | 'grafico'

  const fat = parseFloat(faturamento.replace(/\./g, '').replace(',', '.')) || 0

  const transicao = useMemo(() => {
    if (!fat) return []
    return calcularTransicao(fat, regime, tipo, regimeDif)
  ...
  const todosLabels = [...new Set(transicao.flatMap((a) => a.linhas.map((l) => l.label)))]
    ...
    <div className="max-w-5xl mx-auto px-4 py-8 space-y-8">
      <div>
        ...
        <p className="text-slate-500 text-sm mt-1">
          Veja como a carga tributária evolui ano a ano durante toda a transição
        </p>
      </div>
        ...
        <>
          ...
          <div className="flex gap-2">
            {['tabela', 'grafico'].map((v) => (
              <button key={v} onClick={() => setVisao(v)}
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${visao === v ? 'bg-brand-700 text-white' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
                {v === 'tabela' ? 'Tabela detalhada' : 'Gráfico de barras'}
              </button>
            ))}
          </div>
          ...
          {visao === 'tabela' ? (
              ...
              <table className="w-full text-sm border-separate border-spacing-0">
                <thead>
                  <tr>
                    ...
                    {todosLabels.map((l) => (
                      <th key={l} className="bg-slate-800 text-white text-right px-4 py-3 font-semibold">{l}</th>
                    ))}
                    <th className="bg-slate-800 text-white text-right px-4 py-3 rounded-tr-xl font-semibold">Total / mês</th>
                  </tr>
                </thead>
                <tbody>
                  {transicao.map((linha, i) => {
                    ...
                    return (
                      <tr key={linha.ano} className={`${i % 2 === 0 ? 'bg-white' : 'bg-slate-50'} ${isAtual ? 'ring-2 ring-inset ring-blue-400' : ''}`}>
                        <td className={`sticky left-0 px-4 py-3 font-black text-lg ${isAtual ? 'bg-blue-50 text-blue-700' : i % 2 === 0 ? 'bg-white text-brand-700' : 'bg-slate-50 text-brand-700'}`}>
                          {linha.ano}
                        ...
                        {todosLabels.map((label) => {
                          const item = linha.linhas.find((l) => l.label === label)
                          return (
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 space-y-4">
              <p className="text-xs text-slate-500 mb-2">Carga tributária mensal por tributo (% do faturamento)</p>
              {transicao.map((linha) => (
                <div key={linha.ano} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 flex h-7 rounded-lg overflow-hidden bg-slate-100">
                    {linha.linhas.map((l) => {
                      const pct = (l.aliquota / MAX_BAR) * 100
                      const bg = {
                        PIS: '#cbd5e1', COFINS: '#94a3b8', ICMS: '#fdba74', ISS: '#fdba74',
                        CBS: '#93c5fd', IBS: '#3b82f6', DAS: '#c4b5fd',
                      }[l.label] || '#e2e8f0'
                      return (
                        <div key={l.label} title={`${l.label}: ${fmt(l.valor)} (${fmtPct(l.aliquota)})`}
              ...
              <div className="flex flex-wrap gap-3 pt-2 border-t border-slate-100">
                {[['PIS','#cbd5e1'],['COFINS','#94a3b8'],['ICMS/ISS','#fdba74'],['CBS','#93c5fd'],['IBS','#3b82f6'],['DAS','#c4b5fd']].map(([l,c]) => (
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
            <p className="text-sm font-semibold text-slate-700 mb-3">Impacto anual acumulado (vs. sistema atual)</p>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {transicao.filter((l) => [2026,2027,2029,2033].includes(l.ano)).map((linha) => {
          ...
          <p className="text-xs text-slate-400 flex items-start gap-1.5">
            ...
            Estimativa baseada nas alíquotas de referência. Em 2026 CBS/IBS são compensáveis — sem custo adicional. Para Simples Nacional a composição interna muda a partir de 2029, mas o total permanece próximo ao atual.
          </p>
        </>
      )}
      ...
      {!fat && (
        <div className="text-center py-16 text-slate-400">
          ...
          <p className="font-medium text-slate-500">Informe o faturamento para simular a transição</p>
        </div>
      )}
    </div>
  )
